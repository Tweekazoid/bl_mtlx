...
try:
    ...
    from material_x.blender_materialx_exporter import export_material_to_materialx
    ...
    print("Make sure the material_x directory is in the same directory as this script")
    sys.exit(1)
...
def main():
    ...
    # Optional arguments
    parser.add_argument("--export-textures", action="store_true", help="Export texture files (default: False)")
    parser.add_argument(
        "--texture-path", default="./textures", help="Directory to export textures to (default: ./textures)"
    )
    parser.add_argument("--version", default="1.39", help="MaterialX version (default: 1.39)")
    parser.add_argument(
        "--relative-paths", action="store_true", default=True, help="Use relative paths for textures (default: True)"
    )
    parser.add_argument("--copy-textures", action="store_true", default=True, help="Copy texture files (default: True)")
    parser.add_argument("--active-uvmap", default="UVMap", help="Active UV map name (default: UVMap)")
    parser.add_argument("--blender-path", help="Path to Blender executable (auto-detected if not specified)")
    ...
    success = run_blender_export(
        args.blend_file, args.material_name, args.output_mtlx_file, options, args.blender_path, args.verbose
    )
