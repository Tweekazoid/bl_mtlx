                        ...
                        if hasattr(input_elem, "getConnectedNode"):
                            ...
                            if connected_node and not document.getChild(connected_node.getName()):
                                ...
                                results["warnings"].append(
                                    f"Input '{input_elem.getName()}' on node '{node.getName()}' connects to non-existent node"  # noqa: E501
                                )
                                ...
                                for connection in connections:
                                    if hasattr(connection, "getDownstreamElement"):
                                        ...
                                        if downstream and not document.getChild(downstream.getName()):
                                            results["warnings"].append(
                                                f"Output '{output_elem.getName()}' on node '{node.getName()}' connects to non-existent node"  # noqa: E501
                                            )
                    except Exception:  # noqa: PERF203
...
class MaterialXTypeConverter:
    ...
    def convert_value(self, value: Any, target_type: str) -> Any:  # noqa: PLR0912
        """
...
class MaterialXNodeBuilder:
    ...
    def create_mtlx_input(  # noqa: PLR0917
        self,
        node: mx.Node,
        input_name: str,
        value: Any = None,
